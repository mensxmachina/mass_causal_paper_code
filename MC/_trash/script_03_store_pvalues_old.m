    ...
    for iPop = populationInds
        ...
        parfor iAct = 1:nActivators
           ...
           % fprintf(logFid, '\t\t Activator %s\n', curActivator);
            ...
            ads = load(loadFile);
            actData = ads.dataset.data;
            % fprintf('Population %s, sample size %d\n', curPopulation, size(data,1));
            if size(actData,1) < 10
               % fprintf('Less than 10 activator samples, skipping\n')
                continue;
            end
            loadFile = [matDir, filesep, curInhibitor, filesep, curInhibitor, '_' , curPopulation, '_Ref_'  curDosage,  '.mat'];
            rds = load(loadFile);
            refData = rds.dataset.data;
            if size(refData,1) < 10
                %fprintf('Less than 10 reference samples, skipping\n')
                continue;
            end
